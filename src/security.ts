...
function matchesAllowedHost(url: URL): boolean {
    return config.ALLOWED_HOSTS.some(rawEntry => {
 ...
 */
export function resolveBaseUrl(address?: string): string {
    if (!address) return stripTrailingSlash(config.API_BASE_URL);
    ...
    if (url.origin === configuredOrigin) return resolved;
...
export function displayToken(token: string | null): string | null {
    return config.EXPOSE_TOKEN ? token : maskToken(token);
}
